...
def main():
    ...
    parser.add_argument("--model_name", type=str, default="/mindopt/nlp_nas_2/weiwen/models/Qwen2.5-Math-7B")
    ...
    # Load the dataset
    dataset_name = 'data/math_nsr/train_math.parquet'
    dataset = load_dataset('parquet', data_files=dataset_name, split='train')
    answer_key = "solution"
    tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True)
    dataset = dataset.map(lambda x: prepare_data(x, tokenizer), num_proc=args.dataset_num_proc)
        ...
        # with open(os.path.join(output_dir, output_file), 'w' if args.resume_id == 0 else 'a') as f:
            for i in tqdm(range(0, len(dataset), args.batch_size)):
                ...
                inputs = batch["prompt"]
                answers = [x['ground_truth'] for x in dataset['train']['reward_model']]
