...
if __name__ == "__main__":
    ...
    train_dataset = dataset_math_train.map(function=make_map_fn("train", "hendrycks_math01"), with_indices=True)
    test_dataset_math = dataset_math_test.map(function=make_map_fn("test", "hendrycks_math01"), with_indices=True)
    test_dataset_amc23 = dataset_amc23_test.map(function=make_map_fn("test", "amc2301"), with_indices=True)
    test_dataset_aime2025 = dataset_aime2025_test.map(function=make_map_fn("test", "aime202501"), with_indices=True)
    test_dataset_aime2024 = dataset_aime2024_test.map(function=make_map_fn("test", "aime202401"), with_indices=True)

    local_dir = args.local_dir
    hdfs_dir = args.hdfs_dir

    train_dataset.to_parquet(os.path.join(local_dir, "train_math01.parquet"))
    test_dataset_math.to_parquet(os.path.join(local_dir, "test_math01.parquet"))
    test_dataset_amc23.to_parquet(os.path.join(local_dir, "test_amc2301.parquet"))
    test_dataset_aime2025.to_parquet(os.path.join(local_dir, "test_aime202501.parquet"))
    test_dataset_aime2024.to_parquet(os.path.join(local_dir, "test_aime202401.parquet"))
