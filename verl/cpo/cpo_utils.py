    ...
    for i, (data_item, hint_text) in enumerate(zip(data, hints)):
        ...
        if hint_type == 'gold':
            if "llama" in tokenizer.name_or_path.lower():
                ...
                # breakpoint()
                pad_id = 151643
                question = prompts_text.split("<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n")[1].split("\nPlease reason step by step, and put your final answer within \\boxed{}")[0]
                new_user_prompt = f"""###Question: {question}\n###Reference Answer: {hint_text}\n""" + "Given the above question and its reference answer, your task is to produce a step-by-step explanation that logically leads to the reference answer and put your final answer within \\boxed{}. You are encouraged to draw on the reference answer for internal guidance to help structure and support your reasoning, but the final monologue must read as a genuine, first-encounter, real-time discovery, without mentioning or implying any prior access to the reference answer.\n###OUTPUT REQUIREMENTS:\n1. Output ONLY the first-person, think-aloud monologue. Do not include any preface, summary, or restatement of these instructions.\n2. Maintain the tone of a focused individual thinking to themself. Avoid meta-commentary like “for the first time,” and any phrasing that reveals simulation.\n3. Do not mention, imply, or hint at prior access to the reference answer in the monologue. Avoid phrases like “according to the answer. . . ” or “to get to that answer. . . ”, and any euphemism that signals foreknowledge.\n4. Do not merely restate the reference answer in the monologue; articulate the reasoning pathway with sufficient intermediate steps, rationale, decision points, verification, and any necessary error-correction or backtracking.\n5. Put your final solution value within \\boxed{}."
                new_messages = [
                    {"role":"system", "content": "You are a helpful assistant."},
                    {"role":'user', "content":new_user_prompt},
                ]
                hint_guided_prompt = tokenizer.apply_chat_template(new_messages, add_generation_prompt=True, enable_thinking=False)
                hint_guided_prompt_length = len(hint_guided_prompt)
                # breakpoint()
                if hint_guided_prompt_length > prompt_length:
                    hint_ids = tokenizer.encode(hint_text)[hint_guided_prompt_length - prompt_length + 2:] # 2 is a safety distance
                    hint_text_clip = tokenizer.decode(hint_ids)
                    new_user_prompt = f"""###Question: {question}\n###Reference Answer: {hint_text_clip}\n""" + "Given the above question and its reference answer, your task is to produce a step-by-step explanation that logically leads to the reference answer and put your final answer within \\boxed{}. You are encouraged to draw on the reference answer for internal guidance to help structure and support your reasoning, but the final monologue must read as a genuine, first-encounter, real-time discovery, without mentioning or implying any prior access to the reference answer.\n###OUTPUT REQUIREMENTS:\n1. Output ONLY the first-person, think-aloud monologue. Do not include any preface, summary, or restatement of these instructions.\n2. Maintain the tone of a focused individual thinking to themself. Avoid meta-commentary like “for the first time,” and any phrasing that reveals simulation.\n3. Do not mention, imply, or hint at prior access to the reference answer in the monologue. Avoid phrases like “according to the answer. . . ” or “to get to that answer. . . ”, and any euphemism that signals foreknowledge.\n4. Do not merely restate the reference answer in the monologue; articulate the reasoning pathway with sufficient intermediate steps, rationale, decision points, verification, and any necessary error-correction or backtracking.\n5. Put your final solution value within \\boxed{}."
                    new_messages = [
                        {"role":"system", "content": "You are a helpful assistant."},
                        {"role":'user', "content":new_user_prompt},
                    ]
                    if "-base" in tokenizer.name_or_path.lower():
                        hint_guided_prompt = tokenizer.apply_chat_template(new_messages, add_generation_prompt=True, enable_thinking=True)
                    else:
                        hint_guided_prompt = tokenizer.apply_chat_template(new_messages, add_generation_prompt=True, enable_thinking=False)
                    hint_guided_prompt_length = len(hint_guided_prompt)
                    # if hint_guided_prompt_length > prompt_length:
        ...
        # 处理 input_ids
        hint_guided_prompt_pad = torch.tensor([pad_id] * (prompt_length - hint_guided_prompt_length) + hint_guided_prompt, dtype=torch.int64)
        new_input_ids = torch.cat([hint_guided_prompt_pad, responses], dim=-1)
        assert (new_input_ids[prompt_length:] == original_input_ids[prompt_length:]).all()
        
        ## 处理attention
        hint_guided_attention_pad = torch.tensor([0] * (prompt_length - hint_guided_prompt_length) + [1] * hint_guided_prompt_length, dtype=torch.int64)
        new_attention_mask = torch.cat([hint_guided_attention_pad, response_mask], dim=-1)

        # 处理 position_ids  
        hint_position_ids = torch.arange(hint_guided_prompt_length + response_length, dtype=torch.int64)
        new_position_ids = torch.cat([torch.zeros(prompt_length - hint_guided_prompt_length,  dtype=torch.int64), hint_position_ids])

        # 处理prompts
        new_prompts = hint_guided_prompt_pad
